...
| HTTP client        | axios, not native `fetch`, for both client-side requests and Node.js server-side calls to third-party APIs — not just the browser. Browser: a shared instance in `src/lib/api-client.ts`, wrapped by `src/lib/authenticated-request.ts` for the admin session-refresh retry. Server (Node.js runtime): a shared instance in `src/server/lib/http-client.ts`, used by the OpenAI and Shopify integrations. **Exception:** `src/proxy.ts` runs on the Edge runtime, which has no Node `http`/`net` modules axios depends on — it must keep using `fetch`. |
| Package manager    | npm                                                                                                                                                                                                               |
| Linting/formatting | ESLint + Prettier                                                                                                                                                                                                 |
| Test runner        | Jest (backend unit + integration, and frontend component unit + integration tests), plus Cypress for end-to-end tests                                                                                            |
| Test DB strategy   | Dockerized ephemeral PostgreSQL via `testcontainers` (real Postgres engine, spun up per test run, no cloud dependency, no API keys) for Jest integration tests; the Docker Compose PostgreSQL + a real `next dev` server for Cypress e2e tests |
| Frontend test tools | React Testing Library + `jest-environment-jsdom` for component tests (mocking the HTTP layer via `jest.mock`, not a real server) — see Phase 5                                                                    |
| E2E test tool      | Cypress, run against a real `next dev` server and the Docker Compose PostgreSQL, covering core admin and public user flows end-to-end                                                                            |
...
5. **Automated tests**: unit tests for core business logic (validation, status/visibility rules, auth checks) run without a DB; integration tests for API routes against a `testcontainers` Postgres instance covering the critical scenarios (save validation, draft invisibility, auth gating). Frontend component tests (React Testing Library, `jsdom`) split the same way: unit tests for presentational/validation logic, integration tests for a component's full interaction with a mocked HTTP layer (success, server-rejected save, session-expired, network error). End-to-end tests (Cypress) run against a real `next dev` server and the Docker Compose PostgreSQL, covering the core admin flows (login/logout, already-authenticated redirect, product list, edit-and-save with validation, auth gating on direct navigation) and public flows (catalog visibility, product page content and SEO meta tags, draft unreachable by direct URL) — including the cross-cutting flow where publishing/unpublishing a product in the admin editor changes what the public catalog shows. Test strategy and rationale documented in README.md.

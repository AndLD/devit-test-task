...
| Client-side HTTP   | axios for all client-side requests (not native `fetch`) — used directly via a shared instance in `src/lib/api-client.ts`, and wrapped by `src/lib/authenticated-request.ts` for the admin session-refresh retry   |
| Package manager    | npm                                                                                                                                                                                                               |

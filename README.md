...
`OpenAiProvider`'s own request-building, status-code handling, and response-shape validation are unit-tested against a fake HTTP client in `tests/unit/openai-provider.test.ts` (an injectable `http` constructor parameter, the same DI pattern used throughout the codebase, rather than mocking the module).
...
- **Known code duplication** (identified, not yet fixed — flagged for a future pass rather than fixed unprompted):
  - An identical `truncate(value, max)` helper is defined separately in [mock-provider.ts](src/server/services/llm/mock-provider.ts) and [mapper.ts](src/server/services/shopify/mapper.ts).
  - [suggestion-service.ts](src/server/services/llm/suggestion-service.ts)'s `sanitize()` inlines a similar-but-not-identical truncate step (trims then slices, vs. the helper above's slice-then-trim) — same intent, subtly different edge-case behavior on strings with trailing whitespace right at the limit.
  - The same axios response-handling shape — a `try { ... } catch { throw new <X>Error("Could not reach...") }` network-error wrapper immediately followed by an identical `if (response.status < 200 || response.status >= 300)` range check — is repeated across [openai-provider.ts](src/server/services/llm/openai-provider.ts), [shopify-client.ts](src/server/services/shopify/shopify-client.ts), and [token-provider.ts](src/server/services/shopify/token-provider.ts).
  - See [CODE-QUALITY-REVIEW.md](CODE-QUALITY-REVIEW.md) for the fuller codebase-quality analysis these were found during.
